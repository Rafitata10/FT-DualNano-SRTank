...
CPWindow1::timer1_EvOnTime (CControl * control)
{
 ...
 if(volume > 0){
    if(agit){
       temperatura += (1.0 / (volume * densi * c_esp))*(paque *   2  * 0.1 - presf * resf * 0.1 - 1000.0 * (temperatura - temp0));
       revolutions = 100;
    } else {
...
CPWindow1::menu1_File_DefaultConfig_EvMenuActive (CControl * control)
{
 ...
 edit3.SetText (lxT ("1000"));
 edit4.SetText (lxT ("1000"));
 edit5.SetText (lxT ("3000"));
 edit6.SetText (lxT ("25"));
...
{
 Message (lxT ("Developed by Rafael RS\n <rafars.01.rrs@gmail.com>\n New Version"));
}
